...
{ 
	...
	uint16_t value = W25Q256JV_FSR_QE | (W25Q256JV_FSR_QE << 8);
	...
	/* 传输数据 */
	if (HAL_QSPI_Transmit(&QSPIHandle, (uint8_t *)&value, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
	{
		return QSPI_ERROR;
	}
	/* 自动轮询模式等待存储器就绪 */  
...
static uint8_t QSPI_Addr_Mode_Init(void)
{
	...
	/* 检查寄存器的值 */
	if((reg & W25Q256FV_FSR_4ByteAddrMode) == 1 || ((reg >> 8) & W25Q256FV_FSR_4ByteAddrMode) == 1)    // 4字节模式
	{
    return QSPI_OK;
	}
  else    // 3字节模式
...
static uint8_t QSPI_WriteEnable()
{
	...
	/* 配置自动轮询模式等待写启用 */  
	s_config.Match           = W25Q256JV_FSR_WREN | (W25Q256JV_FSR_WREN << 8);
	s_config.Mask            = W25Q256JV_FSR_WREN | (W25Q256JV_FSR_WREN << 8);
